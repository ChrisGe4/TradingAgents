...
print(
  f"✓ Broker order created: {order.symbol} {order.side.value} {order.quantity}")
...
print(
  f"✓ Broker position: {position.symbol} {position.quantity} shares @ ${position.avg_entry_price}")
print(
  f"✓ Market value: ${position.market_value}, P&L: ${position.unrealized_pnl}")
...
if alpaca_key and alpaca_secret:
  ...
  try:
    ...
    print(
      "  (This is expected if API keys are invalid or network is unavailable)")
else:
...
for signal in test_signals:
  ...
  if order:
    print(
      f"✓ Signal '{signal}' → Broker order: {order.side.value} {order.quantity} {order.symbol}")
  else:
