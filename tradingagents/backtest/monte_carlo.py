...
class MonteCarloSimulator:
  ...
  def __init__(self, config: MonteCarloConfig):
    ...
    logger.info(
      f"MonteCarloSimulator initialized with {config.n_simulations} simulations")
  ...
  def simulate(
    ...
    try:
      if self.config.method == 'resample_returns':
        ...
        simulated_values = self._parametric_simulation(equity_curve,
                                                       initial_value)

      else:
        raise MonteCarloError(
          f"Unknown simulation method: {self.config.method}")
  ...
  def _resample_returns(
    ...
    for i in tqdm(range(self.config.n_simulations),
      ...
      if self.config.preserve_order:
        ...
        # Random resampling
        resampled_returns = np.random.choice(returns, size=n_periods,
                                             replace=True)
  ...
  def _resample_trades(
    ...
    for i in tqdm(range(self.config.n_simulations),
      ...
      if self.config.preserve_order:
        ...
        # Random resampling
        resampled_returns = np.random.choice(trade_returns, size=n_trades,
                                             replace=True)
  ...
  def _calculate_statistics(
    ...
    prob_profit = np.sum(simulated_values > initial_value) / len(
      simulated_values)
  ...
  def simulate_paths(
    ...
    for i in range(n_paths):
      # Resample returns
      resampled_returns = np.random.choice(returns.values, size=n_periods,
                                           replace=True)
