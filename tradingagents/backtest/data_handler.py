...
class HistoricalDataHandler:
  ...
  def load_data(
    ...
    logger.info(
        f"Loading data for {len(tickers)} ticker(s) from {start_date} to {end_date}")
  ...
  def _load_from_yfinance(
    ...
    buffer_start = (datetime.strptime(start_date, "%Y-%m-%d") - timedelta(
  ...
  def _load_from_csv(
    ...
    csv_path = Path(
        self.config.custom_params.get('csv_dir', 'data')) / f"{ticker}.csv"
    ...
    for col in ['open', 'high', 'low', 'close']:
      if (data[col] <= 0).any():
        raise DataQualityError(
            f"Non-positive prices found in {col} for {ticker}")
  ...
  def set_current_time(self, timestamp: datetime) -> None:
    ...
    if self.current_time and timestamp < self.current_time:
      logger.warning(
          f"Time moving backwards: {self.current_time} -> {timestamp}")
    ...
    try:
      ...
      if method == 'inner':
        ...
        raise NotImplementedError(
            f"Alignment method '{method}' not implemented")
      else:
  ...
  def get_corporate_actions(
    ...
    try:
      ...
      if not splits.empty:
        splits = splits[
          (splits.index >= start_date) & (splits.index <= end_date)]
  ...
  def summary(self) -> Dict[str, Any]:
    ...
    for ticker, data in self.data.items():
      summary_dict[ticker] = {
          ...
          "periods": len(data),
          "missing_data_pct": data.isnull().sum().sum() / (
