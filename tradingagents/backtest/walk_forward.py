...
class WalkForwardAnalyzer:
    ...
    while True:
      ...
      if self.config.anchored:
        ...
        current_start = current_start + timedelta(
          days=self.config.step_months * 30)
  ...
  def _combine_oos_metrics(self,
      windows: List[WalkForwardWindow]) -> PerformanceMetrics:
    ...
    oos_metrics = [w.out_sample_metrics for w in windows if
                   w.out_sample_metrics]
    ...
    combined = PerformanceMetrics(
        ...
        max_drawdown_duration=int(
          np.mean([m.max_drawdown_duration for m in oos_metrics])),
  ...
  def _calculate_efficiency_ratio(self,
      windows: List[WalkForwardWindow]) -> float:
    """
