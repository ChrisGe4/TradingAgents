...
class PerformanceAnalyzer:
  ...
  def _calculate_calmar_ratio(self, annualized_return: float,
  ...
  def _calculate_trade_statistics(self, trades: pd.DataFrame) -> Dict[str, Any]:
    ...
    gross_profit = float(winning_trades.sum()) if len(
      winning_trades) > 0 else 0.0
    gross_loss = abs(float(losing_trades.sum())) if len(
      losing_trades) > 0 else 0.0
