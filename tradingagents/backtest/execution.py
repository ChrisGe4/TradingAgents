...
class Fill:
  ...
  def to_dict(self) -> Dict[str, Any]:
    ...
    return {
        ...
        'ticker': self.ticker,
        'side': self.side.value if isinstance(self.side,
                                              OrderSide) else self.side,
        'quantity': float(self.quantity),
    ...
    if order.side == OrderSide.BUY:
      ...
      if total_required > available_capital:
        if self.config.partial_fills:
          ...
          affordable_quantity = available_capital / (
                fill_price * (Decimal("1") + self.config.commission))
          fill_quantity = min(fill_quantity,
                              affordable_quantity.quantize(Decimal("1")))
