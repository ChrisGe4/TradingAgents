...
from .execution import ExecutionSimulator, Order, OrderSide, Fill, \
  create_market_order
  ...
  def update_position(
    ...
    if fill.side == OrderSide.BUY:
      ...
      if position.quantity >= 0:  # Was long or flat
        ...
        position.avg_entry_price = total_cost / new_quantity if new_quantity > 0 else Decimal(
          "0")
      else:  # Was short, closing
        if new_quantity >= 0:  # Fully closed or reversed
          realized_pnl = (position.avg_entry_price - fill.price) * abs(
            position.quantity)
      ...
      if position.quantity > 0:  # Was long
        if new_quantity <= 0:  # Fully closed or reversed
          realized_pnl = (
                               fill.price - position.avg_entry_price) * position.quantity
          self._record_trade(ticker, realized_pnl, fill)
        ...
        total_cost += fill.quantity * fill.price
        position.avg_entry_price = total_cost / abs(
          new_quantity) if new_quantity < 0 else Decimal("0")
...
class Backtester:
    ...
    for current_date in tqdm(trading_days, desc="Backtesting",
      ...
      for ticker in tickers:
        try:
          ...
          logger.warning(
            f"Failed to get data for {ticker} at {current_date}: {e}")
          continue
  ...
  def _process_signal(
    ...
    current_volume = Decimal(
      str(current_bar['volume'])) if 'volume' in current_bar else Decimal("0")
