...
from .local import get_YFin_data, get_finnhub_news, \
  get_finnhub_company_insider_sentiment, \
  get_finnhub_company_insider_transactions, get_simfin_balance_sheet, \
  get_simfin_cashflow, get_simfin_income_statements, get_reddit_global_news, \
...
from .finnhub import (
  ...
  get_news as get_finnhub_news_api,
  get_insider_sentiment as get_finnhub_insider_sentiment_api
)
...
VENDOR_METHODS = {
    ...
    "get_insider_sentiment": {
        "finnhub": get_finnhub_insider_sentiment_api,
...
def route_to_vendor(method: str, *args, **kwargs):
  ...
  logger.debug(
      f"{method} - Primary: [{primary_str}] | Full fallback order: [{fallback_str}]")
  ...
  for vendor in fallback_vendors:
    if vendor not in VENDOR_METHODS[method]:
      if vendor in primary_vendors:
        logger.info(
            f"Vendor '{vendor}' not supported for method '{method}', falling back to next vendor")
      continue
    ...
    logger.debug(
        f"Attempting {vendor_type} vendor '{vendor}' for {method} (attempt #{vendor_attempt_count})")

    # Handle list of methods for a vendor
    if isinstance(vendor_impl, list):
      vendor_methods = [(impl, vendor) for impl in vendor_impl]
      logger.debug(
          f"Vendor '{vendor}' has multiple implementations: {len(vendor_methods)} functions")
    else:
    ...
    for impl_func, vendor_name in vendor_methods:
      try:
        ...
        logger.info(
            f"{impl_func.__name__} from vendor '{vendor_name}' completed successfully")

      except AlphaVantageRateLimitError as e:
        if vendor == "alpha_vantage":
          logger.warning(
              f"Alpha Vantage rate limit exceeded, falling back to next available vendor. Details: {e}")
        # Continue to next vendor for fallback
        continue
      except Exception as e:
        # Log error but continue with other implementations
        logger.error(
            f"{impl_func.__name__} from vendor '{vendor_name}' failed: {e}")
        continue
    ...
    if vendor_results:
      ...
      if len(primary_vendors) == 1:
        logger.debug(
            f"Stopping after successful vendor '{vendor}' (single-vendor config)")
        break
    else:
  ...
  if not results:
    ...
    logger.info(
        f"Method '{method}' completed with {len(results)} result(s) from {vendor_attempt_count} vendor attempt(s)")
