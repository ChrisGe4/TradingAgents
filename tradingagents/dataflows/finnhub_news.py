...
def get_news(
  ...
  response = _make_api_request(client.company_news, ticker, _from=start_date,
                               to=end_date)
...
def get_global_news(
  ...
  """
  logger.info(f"Fetching global news up to {curr_date}")
  client = get_client()
  response = _make_api_request(client.general_news, 'general', min_id=0)

  if not response:
    logger.info("No global news found.")
    return ""

  logger.info(f"Found {len(response)} global news items.")
  result_str = ""
  count = 0
  for item in response:
    if count >= limit:
...
def get_insider_sentiment(
  ...
  response = _make_api_request(client.stock_insider_sentiment, ticker,
...
def get_insider_transactions(
  ...
  logger.info(
      f"Found {len(response['data'])} insider transactions for {symbol}")
  result_str = ""
