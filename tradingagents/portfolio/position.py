...
class Position:
  ...
  def __post_init__(self):
    ...
    if self.take_profit is not None and not isinstance(self.take_profit,
                                                       Decimal):
  ...
  def update_quantity(self, quantity_delta: Decimal) -> None:
    ...
    if (self.is_long and new_quantity < 0) or (
        self.is_short and new_quantity > 0):
      raise InvalidQuantityError(
  ...
  def update_cost_basis(
    ...
    if (self.is_long and quantity_delta > 0) or (
        self.is_short and quantity_delta < 0):
  ...
  def from_dict(cls, data: Dict[str, Any]) -> 'Position':
    ...
    try:
      return cls(
          ...
          stop_loss=Decimal(data['stop_loss']) if data.get(
            'stop_loss') else None,
          take_profit=Decimal(data['take_profit']) if data.get(
