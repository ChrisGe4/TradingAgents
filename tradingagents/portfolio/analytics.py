...
class PerformanceAnalytics:
  ...
  def calculate_trade_statistics(
    ...
    try:
      ...
      # Win rate
      win_rate = Decimal(num_wins) / Decimal(
        total_trades) if total_trades > 0 else Decimal('0')

      # Profit factor
      gross_profit = sum(t.pnl for t in winning_trades)
      gross_loss = abs(sum(t.pnl for t in losing_trades))
      profit_factor = gross_profit / gross_loss if gross_loss > 0 else Decimal(
        '0')
      ...
      largest_loss = abs(
        min((t.pnl for t in losing_trades), default=Decimal('0')))
  ...
  def generate_performance_metrics(
    ...
    try:
      ...
      # Volatility
      volatility = self.calculate_volatility(returns) if returns else Decimal(
        '0')
      ...
      sharpe = risk_manager.calculate_sharpe_ratio(returns,
                                                   risk_free_rate) if returns else Decimal(
        '0')
  ...
  def calculate_rolling_sharpe(
    ...
    try:
      ...
      for i in range(window_days - 1, len(returns)):
        ...
        sharpe = risk_manager.calculate_sharpe_ratio(window_returns,
                                                     risk_free_rate)
