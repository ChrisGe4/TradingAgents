...
class Portfolio:
  ...
  def _execute_buy_order(
    ...
    if check_risk:
      ...
      if order.ticker in self.positions:
        current_position_value = self.positions[order.ticker].market_value(
          current_price)
  ...
  def _execute_sell_order(
    ...
    pnl_percent = pnl / cost_basis_value if cost_basis_value > 0 else Decimal(
      '0')
  ...
  @classmethod
  def load(cls, filename: str = 'portfolio_state.json',
      persist_dir: Optional[str] = None) -> 'Portfolio':
    """
    ...
    # Restore peak value
    portfolio.peak_value = portfolio_data.get('peak_value',
  ...
  def summary(self) -> Dict[str, Any]:
    ...
    with self._lock:
      ...
      return {
          ...
          'total_return': str(
            (total_val - self.initial_capital) / self.initial_capital),
