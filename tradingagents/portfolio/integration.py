...
class TradingAgentsPortfolioIntegration:
  ...
  def get_portfolio_context(
    ...
    try:
      ...
      unrealized_pnl = self.portfolio.unrealized_pnl(
        current_prices) if current_prices else Decimal('0')
      ...
      context = {
          ...
          'cash_pct': str(
            self.portfolio.cash / total_value if total_value > 0 else Decimal(
              '1')),
          ...
          'total_return': str((
                                    total_value - self.portfolio.initial_capital) / self.portfolio.initial_capital),
    ...
    try:
      ...
      for ticker, target_value in target_values.items():
        ...
        if current_position and ticker in current_prices:
          current_value_ticker = current_position.market_value(
  ...
  def _create_order(
    ...
    if order_type == 'market':
      ...
      return LimitOrder(ticker=ticker, quantity=quantity,
