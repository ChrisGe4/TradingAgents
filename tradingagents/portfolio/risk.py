...
from .exceptions import RiskLimitExceededError, CalculationError, \
  ValidationError
...
class RiskManager:
  ...
  def calculate_position_size(
    ...
    if position_value > portfolio_value * self.limits.max_position_size:
      ...
      position_size = (
                            portfolio_value * self.limits.max_position_size) / entry_price
  ...
  def calculate_max_drawdown(self, equity_curve: List[Decimal]) -> Tuple[
    Decimal, int, int]:
    """
  ...
  def calculate_beta(
    ...
    try:
      ...
      covariance = sum(
          (portfolio_returns[i] - port_mean) * (
                benchmark_returns[i] - bench_mean)
          for i in range(n)
      ) / n
