...
def example_basic_trading():
  ...
  print(
    f"Return: {((total_value - portfolio.initial_capital) / portfolio.initial_capital):.2%}")
...
def example_order_types():
  ...
  print("\n--- Limit Order ---")
  limit_order = LimitOrder('GOOGL', Decimal('50'),
                           limit_price=Decimal('2000.00'))

  # Try at higher price - won't execute
  print(
    f"Current price $2050.00 - can execute: {limit_order.can_execute(Decimal('2050.00'))}")

  # Try at limit price - will execute
  print(
    f"Current price $2000.00 - can execute: {limit_order.can_execute(Decimal('2000.00'))}")
...
def example_risk_management():
  ...
  print("Risk Limits:")
  print(
    f"  Max Position Size: {portfolio.risk_manager.limits.max_position_size:.1%}")
  print(
    f"  Max Sector Concentration: {portfolio.risk_manager.limits.max_sector_concentration:.1%}")
  print(f"  Max Drawdown: {portfolio.risk_manager.limits.max_drawdown:.1%}")
  print(
    f"  Min Cash Reserve: {portfolio.risk_manager.limits.min_cash_reserve:.1%}")
...
def example_persistence():
  ...
  portfolio.execute_order(MarketOrder('AAPL', Decimal('100')),
                          Decimal('150.00'))
  portfolio.execute_order(MarketOrder('GOOGL', Decimal('50')),
                          Decimal('2000.00'))
...
def example_tradingagents_integration():
  ...
  rebalance_results = integration.rebalance_portfolio(target_weights,
                                                      current_prices)
  ...
  for result in rebalance_results:
    if result['status'] == 'success':
      print(
        f"  {result['action']} {result['ticker']}: {result['quantity']} shares")
