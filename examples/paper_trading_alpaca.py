...
def main():
  ...
  if positions:
    for pos in positions:
      ...
      print(
        f"      Current: ${pos.current_price} | P&L: ${pos.unrealized_pnl:,.2f}")
  else:
  ...
  try:
    ...
    if updated_order:
      ...
      if updated_order.filled_qty > 0:
        print(
          f"   Filled: {updated_order.filled_qty} @ ${updated_order.filled_price}")
  ...
  if aapl_position:
    ...
    print(
      f"   P&L: ${aapl_position.unrealized_pnl:,.2f} ({aapl_position.unrealized_pnl_percent:.2%})")
  else:
