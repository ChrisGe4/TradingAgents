...
def test_broker_portfolio_integration():
  ...
  try:
    ...
    from tradingagents.portfolio.orders import Order, \
      OrderType as PortfolioOrderType
    ...
    print(
      f"   ✓ Broker order created: {broker_order.symbol} {broker_order.side.value} {broker_order.quantity}")
    ...
    print(
      f"   ✓ Portfolio order created: {portfolio_order.symbol} {portfolio_order.side} {portfolio_order.quantity}")
    ...
    print(
      f"   ✓ Broker position: {broker_position.symbol} @ ${broker_position.avg_entry_price}")
...
def test_configuration_management():
  ...
  try:
    ...
    print(
      f"   ✓ Found {found}/{len(required_sections)} required configuration variables")
    ...
    if configured == 0:
      print(
        "   ℹ No API keys configured - this is expected for fresh installations")
...
def test_data_flow_integration():
  ...
  try:
    ...
    print(
      f"   ✓ Order created: {order.symbol} {order.side.value} {order.quantity}")
    print(f"   ✓ Order type: {order.order_type.value}")

    # Test 4.3: Portfolio update flow
    print("\n4.3: Testing portfolio update flow...")
    from tradingagents.portfolio import Portfolio
    from tradingagents.portfolio.orders import Order as PortfolioOrder, \
      OrderType as POrderType
    ...
    print(
      f"   ✓ Portfolio order created: {test_order.symbol} {test_order.side} {test_order.quantity}")
...
def main():
  ...
  # Run all integration tests
  results.append(("LLM Factory + TradingAgents",
                  test_llm_factory_tradingagents_integration()))
