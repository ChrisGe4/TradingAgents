...
async def main(message: cl.Message):
  ...
  if not parts:
    await cl.Message(
      content="Please enter a command. Type `help` for options.").send()
    return
  ...
  # Buy command
    ...
    if len(parts) < 3:
      await cl.Message(
        content="Usage: `buy TICKER QUANTITY`\n\nExample: `buy AAPL 10`").send()
      return
  ...
  # Sell command
    ...
    if len(parts) < 3:
      await cl.Message(
        content="Usage: `sell TICKER QUANTITY`\n\nExample: `sell AAPL 10`").send()
      return
...
async def analyze_stock(ticker: str):
  ...
  msg = cl.Message(
    content=f"🔍 Analyzing **{ticker}** with TradingAgents...\n\nThis may take 1-2 minutes...")
  await msg.send()
...
async def connect_broker():
  ...
  if cl.user_session.get("broker_connected"):
    await cl.Message(
      content="✓ Already connected to Alpaca paper trading!").send()
    return
...
async def execute_buy(ticker: str, quantity: Decimal):
  ...
  msg = cl.Message(
    content=f"🔄 Placing buy order for {quantity} shares of {ticker}...")
  await msg.send()
...
async def execute_sell(ticker: str, quantity: Decimal):
  ...
  msg = cl.Message(
    content=f"🔄 Placing sell order for {quantity} shares of {ticker}...")
  await msg.send()
...
async def set_provider(provider: str):
  ...
  if provider not in ["openai", "anthropic", "google"]:
    await cl.Message(
      content="❌ Invalid provider. Choose: openai, anthropic, or google").send()
    return
