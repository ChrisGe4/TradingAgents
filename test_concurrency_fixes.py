...
def test_thread_safe_connection():
  ...
  if errors:
    ...
    results.add_fail("test_thread_safe_connection",
  ...
  if broker.connected:
    ...
    results.add_fail("test_connection_state_consistency",
                     "Broker not connected after threads")
...
def test_concurrent_api_calls():
  ...
  if errors:
    ...
    results.add_fail("test_concurrent_api_calls",
                     f"Only {len(accounts)}/5 succeeded")
  else:
...
def test_session_cleanup():
  ...
  if not broker.connected:
    ...
    results.add_fail("test_connection_flag_cleared",
                     "Connection still marked as active")
...
def test_no_global_state():
  ...
  try:
    ...
    if 'global ta_graph' in content or 'global broker' in content:
      results.add_fail("test_no_global_state",
                       "Found global declarations in web_app.py")
    else:
    ...
    if 'cl.user_session.get(' in content and 'cl.user_session.set(' in content:
      ...
      results.add_fail("test_session_usage",
                       "Session storage not used properly")
